...
class SecretController extends Controller
{
    ...
    public function store(Request $request) {
        ...
        $expiresAt = Carbon::now('Europe/Budapest')->addMinutes($data['expireAfter'])->format('Y-m-d H:i:s.v');
        ...
        //Object creation and DB insertion, furthermore the response message and status code
        Secret::create($fields);
        return $responseFormat->format(['message' => 'Successfully added a secret!'], 201);
    }
    ...
    public function details(Secret $secret, Request $request) {
        ...
        //Checking if the current secret is viewable (views still remaining and it is not past the expiry date)
        $remainingViews = $secret->remainingViews;
        if($remainingViews <= 0){
            return $responseFormat->format(['error' => 'The chosen secret can not be viewed anymore!'], 403);
        }

        $expiresAt = Carbon::parse($secret->expiresAt)->format('Y-m-d H:i:s.v');
        if($expiresAt <= Carbon::now('Europe/Budapest')) {
            return $responseFormat->format(['error' => 'The chosen secret has expired!'], 403);
        }
        
        //Decreasing the reamining views by 1
        $secret->update(['remainingViews' => $remainingViews-1]);
        //Returning the secret either as an xml or a json response with the '200' status code
